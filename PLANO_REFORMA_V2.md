...
**Versão:** 2.3 (Fase 3 — proposta de endurecimento de RLS)
**Data:** 2026-07-07
...
- [~] Revisar RLS: substituir políticas `USING (true)` por políticas efetivas. — **Fase 3 proposta (2026-07-08):** [SECURITY_RLS_PHASE3_PLAN_V2.md](SECURITY_RLS_PHASE3_PLAN_V2.md) + scripts em [supabase/security-hardening-proposals/phase3-rls/](supabase/security-hardening-proposals/phase3-rls/) (helpers `is_admin`/`has_secao` + 5 lotes por grupo A-E, cada um com rollback). Policies visam `authenticated` (bloqueia anon) por papel/seção. **Só aplicável após a Fase 2 validada** (depende de `auth.uid()`). Nada aplicado.
