...
- [x] **Fase 2 — EXECUTADA e VALIDADA em local (2026-07-09).** Aplicada a infra no projeto novo (`ewfebwljhmcvuopopqpb`): (1) `0001_auth_link_and_profile.sql` aplicado (coluna `auth_user_id`, FK→`auth.users`, RPC `get_my_profile` `SECURITY DEFINER`); (2) `link-migrate-users.mjs` rodado com `service_role` efêmera no terminal → **5/5 usuários** criados no Supabase Auth e vinculados (`auth_user_id` preenchido, sem vínculos fantasma); (3) Edge Function **`auth-bridge` deployada** (código versionado em [supabase/functions/auth-bridge/index.ts](supabase/functions/auth-bridge/index.ts), `verify_jwt=false` em [config.toml](supabase/config.toml)); (4) flag `VITE_AUTH_MODE=supabase` só no `.env.local`. **Testes:** login admin (kaiomelo) OK; **1º login do rh** via bridge OK (após corrigir **CORS** na função — a 1ª versão deployada não tinha headers/OPTIONS, o navegador bloqueava; `index.ts` agora responde ao preflight); telas carregam; **fallback para `custom` validado** (remover a flag → login antigo volta na hora). `service_role`/segredos **nunca** versionados; `.env.local` git-ignored; `supabase/.temp/` adicionado ao `.gitignore`. **Próximo:** Fase 3 (RLS) já aplicável — `auth.uid()` agora funciona.
- [x] **Fase 3 — RLS APLICADA e VALIDADA (2026-07-09).** Os 5 lotes de [supabase/security-hardening-proposals/phase3-rls/](supabase/security-hardening-proposals/phase3-rls/) aplicados no projeto novo, um a um: `0001` helpers (`is_admin`/`has_perfil`/`has_secao`/`current_secoes`, todos `SECURITY DEFINER` sobre `auth.uid()`→`auth_user_id`), `0002` sensíveis (funcionarios/resultados/faltas/avaliacoes/historico/usuarios), `0003` operacionais (dss/epi/producao_setor/indicadores_gerais/indicadores_setor), `0004` cadastros+config+cargos (15 tabelas), `0005` permissões (user_roles/user_application_permissions). **Todas as 27 policies `allow_all` (to public) removidas** → **28 tabelas** com anon bloqueado. **Testes REST anon:** leitura das 28 tabelas retorna `[]` (0 vazamentos; baseline vazava funcionarios+CPF, resultados, dss, setores…); escrita anon em `funcionarios`/`resultados_premiacao` negada (`42501`, HTTP 401). **Leitura autenticada (admin via GoTrue):** funcionarios/usuarios/resultados/setores/dss retornam dados (prova `is_admin()=true`). `typecheck` limpo, `test` 32/32, `build` OK. **Nenhum rollback usado. Risco C2 fechado.** Pendente: validação manual das telas por perfil não-admin (rh/sesmt/producao) — a cargo do operador; e restrição a **nível de coluna** (salário/CPF em funcionarios ainda legível por autenticado) fica como passo futuro (VIEW/coluna-grant).
- [ ] Avaliar migração da autenticação client-side (`localStorage`) para Supabase Auth ou tokens assinados. — *Fases 0/2 do audit*
- [x] Cortar vetores críticos das RPCs de gestão de usuário (revoke/authz). — **Fase 4 APLICADA e VALIDADA (2026-07-09):** [SECURITY_PHASE4_RPC_REVOKE_PLAN_V2.md](SECURITY_PHASE4_RPC_REVOKE_PLAN_V2.md) + [phase4-rpc-revoke/](supabase/security-hardening-proposals/phase4-rpc-revoke/). `revoke execute (public+anon) / grant (authenticated+service_role)` em 6 RPCs `SECURITY DEFINER` (`create_user`, `update_user_password`, `update_funcionario_setor_ids`, `get_all_funcionario_setor_ids`, `get_my_perfil`, `get_my_profile`). Fechou o **vazamento** de `get_all_funcionario_setor_ids` (devolvia funcionario_id/setor_ids a anon, furando a RLS da Fase 3). `concremrh_verify_login` **mantida para anon** (login/fallback; `auth-bridge` usa service_role) — decisão documentada. **Testes:** anon → `42501` "permission denied" nas 6 (HTTP 401); `verify_login` 200; authenticated (admin) → `get_my_profile`/`get_my_perfil`/`get_all_funcionario_setor_ids` (208 linhas) OK e `create_user` executa mantendo o gate da Fase 1. `typecheck`/`test` 32/32/`build` verdes. Rate limit documentado como config de Dashboard. Sem rollback.
- [~] Proteger colunas sensíveis de `funcionarios` (salário/PII). — **Fase 5A PROPOSTA (2026-07-09):** [SECURITY_PHASE5A_FUNCIONARIOS_COLUMNS_PLAN_V2.md](SECURITY_PHASE5A_FUNCIONARIOS_COLUMNS_PLAN_V2.md) + [phase5a-funcionarios-columns/](supabase/security-hardening-proposals/phase5a-funcionarios-columns/). Achado-chave: `cpf` é o **Código Funcionário** (chave de premiação/Faltas/import) → **mantido**; `salario` é o único sensível, lido só pelo módulo Cargos & Salários; `data_nascimento`/`telefone` não são usados. Proposta: **grant por coluna** na tabela base (esconde salario/email/data_nascimento/telefone de `authenticated`) + view guardada `concremrh_funcionarios_sensivel` (gate por coluna: salário→admin/cargos_salarios; email→admin/rh/cargos_salarios) + view `concremrh_funcionarios_resumo` (sem salario/email/telefone/data_nascimento; mantém cpf). **Decisões:** RH não vê salário por padrão; SESMT/Produção não veem salário nem email. Requer ajuste de hooks (Fase 5B: `useFuncionarios` sai de `select('*')`).
- [x] **Fase 5B — EXECUTADA e VALIDADA (2026-07-09).** Código: novo hook `useFuncionariosSensivel` (consulta a view guardada `concremrh_funcionarios_sensivel`); `useFuncionarios` saiu de `select('*')` → projeção explícita sem `salario`/`email`/`telefone`/`data_nascimento` (mantém `cpf`); telas `cargos-salarios/Funcionarios.tsx` e `cargos-salarios/Dashboard.tsx` migradas para o hook sensível; DSS/EPI/Produção/Indicadores/Gerar/Relatório inalteradas. **SQL 5A aplicado no projeto novo** (grant por coluna + views `_sensivel`/`_resumo`). **Testes REST/SQL:** base `?select=salario|email` → `42501` negado para todo `authenticated` (inclusive admin); view sensível: admin `is_admin=true` (gate libera); anon → view negada (401) e base → 0 linhas. Achado: `salario`/`email` estão **100% nulos** nas 321 linhas (nunca cadastrados) — média salarial 0 é dado real, não regressão. **Telas validadas em todos os perfis** (admin/rh/cargos_salarios/sesmt/producao): dropdowns preenchidos, Gerar premiação calcula. `typecheck`/`test` 32/32/`build` verdes. Branch `fase-5b-funcionarios-columns`.
