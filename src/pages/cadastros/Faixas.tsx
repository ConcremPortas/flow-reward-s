...
export const Faixas = () => {
  ...
  const handleAddFaixa = async () => {
    if (!nomeFaixa.trim()) return;
    
    await createFaixa({
      nome: nomeFaixa,
      valor_minimo: valorMinimo ? parseFloat(valorMinimo) : undefined,
      valor_maximo: valorMaximo ? parseFloat(valorMaximo) : undefined,
      ativo: true
    });
      ...
      <Card className="card-elegant">
        ...
        <CardContent className="space-y-6">
          ...
          {(valorMinimo || valorMaximo) && (
            <div className="p-4 bg-accent/50 rounded-lg">
              <div className="flex items-center justify-between">
                <span className="text-sm font-medium">Faixa de valores:</span>
                <span className="text-lg font-bold text-primary">
                  {formatCurrency(Number(valorMinimo || 0))} - {formatCurrency(Number(valorMaximo || 0))}
                </span>
              </div>
            </div>
          )}
          ...
          <div className="flex justify-end space-x-2">
            ...
            <Button className="gap-2" onClick={handleAddFaixa} disabled={!nomeFaixa.trim()}>
