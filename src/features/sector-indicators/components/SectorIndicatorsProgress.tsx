...
interface Props {
  previstos: number;
  apurados: number;
  pendentesRows: SectorIndicatorRow[];
  onShowPendentes: () => void;
}

/** Progresso da apuração da competência — X de Y setores apurados + pendentes. */
export function SectorIndicatorsProgress({ previstos, apurados, pendentesRows, onShowPendentes }: Props) {
  ...
  return (
    <div className="rounded-xl border border-border/70 bg-card p-4 shadow-[var(--shadow-card)]">
      <div className="flex flex-wrap items-center justify-between gap-2">
        <div>
          <p className="text-sm font-semibold text-foreground">{apurados} de {previstos} setores apurados</p>
