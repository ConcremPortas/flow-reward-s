...
import type { ProductionDirtyDiff } from '../domain/productionComparison';

interface Props {
  open: boolean;
  onOpenChange: (open: boolean) => void;
  competencia: string;
  diff: ProductionDirtyDiff;
...
export function ProductionReviewDialog({ open, onOpenChange, competencia, diff, setorNome, saving, error, onConfirm }: Props) {
  return (
      ...
      <DialogContent className="max-w-2xl">
        <DialogHeader>
          ...
          <DialogDescription>
            ...
            {' · '}{pluralizeBR(diff.metasAlteradas, 'meta', 'metas')} · {pluralizeBR(diff.realizadosAlterados, 'produção', 'produções')}
          </DialogDescription>
        </DialogHeader>
          ...
          <Table>
            ...
            <TableBody>
              {diff.changedFields.map((c, i) => (
                <TableRow key={`${c.setorId}-${c.field}-${i}`}>
