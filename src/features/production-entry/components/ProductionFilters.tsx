...
import { countActiveFilters } from '../domain/productionFilters';
import { UNIDADES_MEDIDA, type ProductionFilters as Filters, type ProductionSituacao } from '../types/production-entry.types';
...
const SITUACOES: ProductionSituacao[] = ['superada', 'proxima', 'abaixo', 'pendente'];

export function ProductionFilters({ searchInput, onSearchChange, filters, onChange, onReset, empresas, setores }: Props) {
    ...
    <div className="space-y-2.5">
      ...
      {active > 0 && (
        <div className="flex flex-wrap items-center gap-1.5">
          ...
          {filters.unidade !== 'todos' && <Chip label={UNIDADES_MEDIDA.find((u) => u.value === filters.unidade)?.label ?? 'Unidade'} onClear={() => onChange({ unidade: 'todos' })} />}
